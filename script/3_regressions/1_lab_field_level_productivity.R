...
ds_clean <- ds_clean %>%
  .[, quartile_n_au_2003 := cut(n_au_2003,
                                quantile(unique(ds_clean[, list(merged_inst_id, field, n_au_2003)])$n_au_2003,
                                         probs = c(0.0,0.25,0.5,0.75,1) 
                                ), include.lowest = T
                                , labels = FALSE
  )] %>%
  .[, quartile_avg_pub_2003 := cut(avg_publications_2003,
                                quantile(unique(ds_clean[, list(merged_inst_id, field, avg_publications_2003)])$avg_publications_2003,
                                         probs = c(0.0,0.25,0.5,0.75,1) 
                                ), include.lowest = T
                                , labels = FALSE
  )] %>%
  
  .[, n_inst_city := n_distinct(paste0(merged_inst_id, field)), by = 'city']


examiner <- unique(ds_clean %>%
                     .[, list(merged_inst_id, field, name, city, type, cnrs,
...
for(var in outcomes){
  ...
  no_ctrl_path = "D:\\panel_fr_res\\productivity_results\\labs_field\\no_ctrl\\"
  ctrl_path = "D:\\panel_fr_res\\productivity_results\\labs_field\\ctrl\\"
