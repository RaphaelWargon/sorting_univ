...
make_stargazer_like_table_dt(rbind(nyt_est_no_ctrl,nyt_est_ctrl),
                             var_map = dict_vars, 
                             treat_map = dict_vars, 
                             pre_mean = pre_mean,
                             n_obs = n_obs,
...
make_stargazer_like_table_dt(rbind(ch_est_no_ctrl,ch_est_ctrl),
                             var_map = dict_vars, 
                             treat_map = dict_vars, 
                             pre_mean = pre_mean,
                             n_obs = n_obs,
...
for(treat in c(#'acces_rce',
  ...
  match_units <- matchit(treat ~
                           entry_year + domain + prod_au_n_tile
                         ,data = units
                         ,method = "exact",
  )
...
for(var in names(list_es_solo[[treat]])){
  ...
  for(treat in unique(agg_stag_by_t_ctrl$treatment)){
    p <- ggplot(agg_stag_by_t_ctrl %>% .[treatment %in% c(treat)])+
      ...
      geom_errorbar(aes(x=t, ymin = est -1.96*std, ymax=est+1.96*std), width = 0.5)+
      geom_vline(aes(xintercept = "-1"), linetype = "dashed")+geom_hline(aes(yintercept = 0))+
      labs(title = paste0('Treatment: ', dict_vars[[treat]], " ", var))+xlab('Time to treatment')+ ylab('Estimate and 95% CI')+
      theme_bw()
