...
rm(ds)
gc()
length(unique(sample_df$author_id)) #828351
nrow(unique(sample_df[, list(merged_inst_id, field)])) #86210
...
sample_df_reg <- sample_df %>%
  ...
  #.[pub_04_07 >=2] %>%
  .[type %in% c('facility','education') & (any_treatment ==1|country == 'GB')] %>%
  .[str_count(field, ',')<=1]%>%
  .[, n_lt := n_distinct(author_id), by = c('merged_inst_id','field','year')] %>%
  .[, merged_inst_id_field := paste0(merged_inst_id, '_', field)] %>%
  .[, fusion_date := fifelse(fusion_date =="2023", "0", as.character(fusion_date))] %>%
...
sample_df_reg <- merge(sample_df_reg, cities %>% .[, city:=LIBELLE], by ='city', allow.cartesian = TRUE, all.x = TRUE)%>%
...
length(unique(sample_df_reg$author_id)) #107495
nrow(unique(sample_df_reg[, list(merged_inst_id, field)])) #24570
table(unique(sample_df_reg[, list(merged_inst_id, field, acces_rce,date_first_idex,fusion_date)])$acces_rce)
table(unique(sample_df_reg[, list(merged_inst_id, field, acces_rce,date_first_idex,fusion_date, country)])$acces_rce,
      unique(sample_df_reg[, list(merged_inst_id, field, acces_rce,date_first_idex,fusion_date, country)])$country
      )

table(unique(sample_df_reg[, list(merged_inst_id, field, acces_rce,date_first_idex,fusion_date, country)])$acces_rce,
      unique(sample_df_reg[, list(merged_inst_id, field, acces_rce,date_first_idex,fusion_date, size_n_tile)])$size_n_tile
)


table(unique(sample_df_reg[, list(merged_inst_id, field, acces_rce,date_first_idex,fusion_date)])$date_first_idex)

table(unique(sample_df_reg[, list(merged_inst_id, field, acces_rce,date_first_idex,fusion_date, country)])$date_first_idex,
      unique(sample_df_reg[, list(merged_inst_id, field, acces_rce,date_first_idex,fusion_date, country)])$country
)
...
for(var in outcomes){
  no_ctrl_path = "D:\\panel_fr_res\\productivity_results\\individual_enfr\\no_ctrl\\"
  ctrl_path = "D:\\panel_fr_res\\productivity_results\\individual_enfr\\ctrl\\"
  ...
  sample_df_reg <- sample_df_reg %>%
  .[, ':='(y_lt = sum(y)), by = c('merged_inst_id_field', 'year')] %>%
...
make_stargazer_like_table_dt(unique(agg_stag %>%
                             ...
                             save_path = 'D:\\panel_fr_res\\productivity_results\\individual\\agg_prod_fren.tex'
)
...
make_stargazer_like_table_dt(agg_stag %>%
                             ...
                             n_obs = n_obs,
                             r_2 = r_2,
                             var_order = c('publications_raw','citations_raw','nr_source_top_10pct_raw','nr_source_top_5pct_raw'), 
                             drop_unlisted_vars = TRUE,
                             save_path = 'D:\\panel_fr_res\\productivity_results\\individual\\agg_prod_raw.tex'
)
...
saveRDS(list_es, file = "D:\\panel_fr_res\\productivity_results\\individual_enfr\\all_regressions.rds")
